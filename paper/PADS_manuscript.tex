...
\begin{itemize}
    ...
    \item \textbf{Go/No-Go 2 (pause-duration feasibility): FAILED, by a razor-thin margin.} The decode-step-timing half was measured directly: wall-clock time for one depth-extension step (drafted tokens $\rightarrow$ full-depth verification) across $n=147$ real steps was mean 285.1\,ms, median 263.2\,ms (interquartile range 256.6--274.4\,ms, long tail to 1062.3\,ms), using the LayerSkip reference (PyTorch/\texttt{transformers}) implementation rather than the faster quantized llama.cpp path -- likely a pessimistic estimate of real deployment-time cost. Switchboard/CallHome pause statistics remain pending LDC corpus access; as a verified, freely-available alternative source of real between-speaker pause data, we extracted gap durations directly from the AMI Meeting Corpus~\cite{carletta2005ami} (CC~BY~4.0, no institutional gating), computing the silence between one speaker's last word and the next speaker's first word at every real speaker transition across 171 meetings (Heldner and Edlund's operational definition of a gap). After excluding likely non-turn-taking lapses ($>$10\,s, 0.38\% of the data), 64{,}173 real gaps had median 260.0\,ms -- closely matching the previously-published ranges already cited in Section~\ref{sec:related} and corroborating the extraction method. Comparing directly: median real pause (260.0\,ms) is 3.2\,ms shorter than the median depth-extension step (263.2\,ms), firing the kill condition of Table~\ref{tab:gonogo}, row~2, by under 1.5\%. We report this honestly as FAILED per the pre-registered criterion rather than rounding it to a pass, while noting two concrete reasons this may not reflect the mechanism's real feasibility: the step-timing measurement used the slower reference implementation rather than the quantized deployment path, and AMI's multi-party meeting register may differ systematically from the dyadic telephone conversations this project's timing premise targets. 49.0\% of real pauses already exceed the measured step time, so exploiting only longer pauses -- the pre-committed fallback for this outcome -- is an evidenced, not merely hypothetical, design option; re-measuring step time on the quantized path is the other immediate next step before treating this as a final verdict. As a rough plausibility check only (not a substitute measurement), the plain-decoding speedup of quantized llama.cpp over the PyTorch reference implementation on this hardware ($\sim$3.5$\times$, from 49.2 vs. 13.9 tok/s measured separately) would, applied naively, put the estimated quantized step time near 74\,ms -- comfortably under essentially all real pauses -- but the depth-extension step's batched-verification computation pattern differs enough from plain sequential decoding that we do not treat this as resolving the question.
...
We have described Pause-Aware Depth Scheduling, a mechanism for triggering self-speculative early-exit depth extension during the pause preceding a conversational turn's end, positioned precisely against the three closest prior systems we identified and scoped into an independently defensible safe-core characterization plus an additive stretch contribution. A pre-registered set of seven falsification experiments is designed to invalidate the premise cheaply before deep implementation investment, consistent with treating this as a checkable empirical claim rather than a vision statement. Five of these seven experiments are now complete with real results (Section~\ref{sec:status}), including an honest near-tie failure on the core timing premise itself (Go/No-Go~2) that we report rather than round to a pass. Remaining work comprises resolving the two open experiments -- Go/No-Go~4, pending a scheduling decision, and completing Go/No-Go~5's full pipeline measurement once the safe-core components exist to measure -- re-collecting all results on the target hardware once available, implementing and benchmarking the safe-core pipeline against reproduced baselines, and -- contingent on an internal go/continue checkpoint informed by how Go/No-Go~2 is ultimately resolved -- implementing and evaluating the pause-time stretch mechanism itself.
...
\bibitem{carletta2005ami} J. Carletta \emph{et al.}, ``The AMI Meeting Corpus: A Pre-Announcement,'' in \emph{Proc. 2nd Int. Workshop Mach. Learn. Multimodal Interact. (MLMI)}, 2005.
