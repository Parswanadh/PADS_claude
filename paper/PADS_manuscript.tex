...
LayerSkip~\cite{elhoushi2024layerskip} trains a single model with layer dropout and a shared early-exit loss so that any layer can produce a usable output, then performs self-speculative decoding by drafting from a shallow exit and verifying against full depth with the same KV cache. Follow-on work refines \emph{which} layers to skip and how the exit decision is made, including training-free dynamic-programming layer selection~\cite{chen2025clasp}, input-adaptive layer sparsity~\cite{xu2025specee}, and quantized-KV-cache self-speculation~\cite{quantspec2025}. Critically, a 2026 systematic study~\cite{wei2026diminishing} re-examines these assumptions on modern (post-2023) model families and reports that early-exit adaptability \emph{decreases} across model generations as pretraining and alignment concentrate decision-relevant computation in later layers, and that dense transformers larger than approximately 20B parameters, together with models explicitly fine-tuned for early exit (as opposed to used off-the-shelf), retain the most exploitable redundancy. This finding directly informs our model-selection and Go/No-Go design (Section~\ref{sec:evaluation}): it argues against assuming an arbitrary small instruction-tuned checkpoint will exhibit LayerSkip-like behavior without dedicated fine-tuning, and motivates measuring early-exit adaptability empirically before committing to a base model.
...
\subsection{The two closest prior systems}
Two independently developed systems sit closer to PADS than the above and warrant explicit differentiation.
...
To our knowledge, no prior system combines turn-level, pre-endpoint predictive triggering with model-\emph{depth} extension within a single self-speculative model evaluated on CPU-only hardware. This is the compound gap PADS targets, precisely bounded by the two systems above on either side of it.
...
3 & Off-the-shelf / fine-tuned early-exit adaptability & Measure logit-similarity-based early-exit adaptability score~\cite{wei2026diminishing} for candidate base and fine-tuned checkpoints before committing to one & Adaptability too low even after fine-tuning; reconsider model family or size per~\cite{wei2026diminishing}'s finding that larger dense models retain more redundancy \\
...
\begin{itemize}
    \item \textbf{Toolchain verification.} The llama.cpp build toolchain was cloned and configured for a CPU-only target (\texttt{-DGGML\_NATIVE=ON}, no CUDA/Metal backend). CMake configuration succeeded without error. During an initial build attempt, the default build target was found to unnecessarily pull in an npm-based embedded web UI component (server-only, irrelevant to headless CLI inference); this was identified and disabled (\texttt{-DLLAMA\_BUILD\_UI=OFF}) as a correction to the build configuration. Compilation of the core inference library subsequently proceeded with zero errors before being halted by an environment time constraint unrelated to build correctness (a single-core verification environment, not the target hardware).
    \item \textbf{Software components implemented and unit-tested.} A conservative, asymmetric trigger-policy module implementing Equation~\ref{eq:trigger} was implemented and validated against a unit test suite (six tests covering threshold boundary conditions, out-of-range input handling, and outcome classification), all passing. A benchmark-harness module implementing the aggregation, confidence-interval, and structured-logging requirements of Section~\ref{sec:evaluation} was implemented and validated via a self-test mode using synthetic timing data, whose numbers are explicitly excluded from any reported result.
    \item \textbf{Not yet executed.} Go/No-Go experiments 1--7 (Table~\ref{tab:gonogo}) require the target hardware and, for experiments 2 and 3, real conversational-pause corpora and real fine-tuned or off-the-shelf early-exit checkpoints, none of which were available in the software-verification environment used for this draft. These constitute the immediate next phase of the study.
\end{itemize}
