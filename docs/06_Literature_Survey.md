...
### H.4 Third system found in the 2026-08-07 biweekly scoop check — same problem space, opposite mechanism

**"Thinking While Speaking: Inference-Time Knowledge Transfer for Responsive and Intelligent Conversational Voice Agents" (arXiv:2511.07397, 2025/2026).** Found via Go/No-Go Test 7's query 3 ("speculative decoding conversational pause utterance latency hiding"). Verified by fetching the abstract directly (not just the search snippet) before logging this. A small, fast "talker" model immediately generates contextually grounded responses to hide the latency of a slower external "reasoner" model, then fluently integrates the reasoner's streamed knowledge into its response as it arrives — closing the accuracy gap to within 6.3% of the frontier reasoner while keeping millisecond-level response times.

*Why it's not PADS:* it targets the same underlying problem (a conversational agent's response is gated on slow reasoning, and there's dead time to exploit) but solves it with exactly the architecture `docs/03_SKILL.md` §0 rules out for this project — a **two-model** talker/reasoner split with knowledge streamed from the large model into the small one. PADS stays single-model (early layers → later layers of the *same* network, LayerSkip-style) and exploits a different idle window (the human's pre-turn-end pause, not the agent's own post-prompt "thinking" time while it is already speaking). Cite as related work and use it to sharpen the manuscript's differentiation between "hide latency via a second model" and "hide latency via depth-extension inside one model," which is arguably PADS's central architectural bet.
